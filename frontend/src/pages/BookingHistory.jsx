...
const BookingHistory = () => {
  ...
  const handleDownloadTicket = async (bookingId, ticketCode) => {
    ...
    try {
      ...
      a.href = url;
      a.download = `ticket-${ticketCode || bookingId}.pdf`;
                    ...
                    <tr key={booking.bookingId} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {booking.status === 'CONFIRMED' && booking.ticketCode ? (
                          <button
                            onClick={() => handleDownloadTicket(booking.bookingId, booking.ticketCode)}
                            disabled={downloadingTicket === booking.bookingId}
                            className="inline-flex items-center px-3 py-1.5 bg-blue-600 text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed rounded-md text-xs font-medium transition-colors"
                            title="Download E-Ticket"
                          >
                            {downloadingTicket === booking.bookingId ? (
                              <>
